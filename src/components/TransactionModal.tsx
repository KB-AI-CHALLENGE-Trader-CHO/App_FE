...
import { StockSymbol } from "../hooks/useSymbolSearch";
...
interface Props {
  ...
  onDelete?: (txId: string, txDate: string) => void;
  date: string; // For create mode
  mode: "create" | "edit";
...
const TransactionModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (visible) {
      if (isEdit && initialData) {
        setStock({ name: initialData.name, symbol: initialData.symbol });
        setType(initialData.type);
        setQuantity(String(initialData.quantity));
        setPrice(String(initialData.price));
        setAvgBuyPrice(
          initialData.avgBuyPrice ? String(initialData.avgBuyPrice) : ""
        );
  ...
  const handleSubmit = () => {
    if (!stock || !quantity || !price) {
      Alert.alert("입력 오류", "종목명, 수량, 단가를 모두 입력해주세요.");
      return;
    }
    ...
    const transactionData = {
      ...
      price: Number(price),
      avgBuyPrice:
        type === "sell" ? Number(avgBuyPrice) || undefined : undefined,
      memo,
    };
...
const styles = StyleSheet.create({
  ...
  dateText: {
    ...
    fontSize: 16,
    color: "#645B4C",
    overflow: "hidden", // for border-radius on iOS
  },
  typeSelector: {
    flexDirection: "row",
    marginBottom: 16,
